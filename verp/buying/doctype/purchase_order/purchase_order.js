...
{% include 'verp/public/js/controllers/buying.js' %};
	...
	get_materials_from_supplier: function(frm) {
		...
		if (po_details && po_details.length) {
			frm.add_custom_button(__('Return of Components'), () => {
				frm.call({
					method: 'verp.buying.doctype.purchase_order.purchase_order.get_materials_from_supplier',
					freeze: true,
...
verp.buying.PurchaseOrderController = class PurchaseOrderController extends verp.buying.BuyingController {
	setup() {
				...
				if (doc.status != "On Hold") {
					...
					if(!doc.auto_repeat) {
						cur_frm.add_custom_button(__('Subscription'), function() {
							verp.utils.make_subscription(doc.doctype, doc.name)
						}, __('Create'))
					}
	...
	get_items_from_open_material_requests() {
		verp.utils.map_current_doc({
			...
			get_query_method: "verp.stock.doctype.material_request.material_request.get_material_requests_based_on_supplier"
		});
	}
	...
	_make_rm_stock_entry(rm_items) {
		frappe.call({
			method:"verp.buying.doctype.purchase_order.purchase_order.make_rm_stock_entry",
			args: {
...
function set_schedule_date(frm) {
	if(frm.doc.schedule_date){
		verp.utils.copy_value_in_all_rows(frm.doc, frm.doc.doctype, frm.doc.name, "items", "schedule_date");
	}
}
