...
{% include 'verp/accounts/doctype/sales_taxes_and_charges_template/sales_taxes_and_charges_template.js' %}
...
verp.selling.SellingController = class SellingController extends verp.TransactionController {
		...
		$.each([["customer", "customer"],
			...
			function(i, opts) {
				if(me.frm.fields_dict[opts[0]])
					me.frm.set_query(opts[0], verp.queries[opts[1]]);
			});
	...
	customer_address() {
		...
		verp.utils.set_taxes_from_address(this.frm, "customer_address", "customer_address", "shipping_address_name");
	}

	shipping_address_name() {
		verp.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
		verp.utils.set_taxes_from_address(this.frm, "shipping_address_name", "customer_address", "shipping_address_name");
	}
		...
		frappe.db.get_value('Item', {'item_code': item.item_code}, 'has_batch_no', (r) => {
			...
			if(item.item_code && item.warehouse) {
				return this.frm.call({
					method: "verp.stock.get_item_details.get_bin_details_and_serial_nos",
					child: item,
					args: {
	...
	batch_no(doc, cdt, cdn) {
		...
		frappe.db.get_value('Item', {'item_code': item.item_code}, 'has_serial_no', (r) => {
			...
			if(item.warehouse && item.item_code && item.batch_no) {
				return this.frm.call({
					method: "verp.stock.get_item_details.get_batch_qty_and_serial_no",
					child: item,
					args: {
	...
	*/
	...
	_set_batch_number(doc) {
		...
		return frappe.call({
			method: 'verp.stock.doctype.batch.batch.get_batch_no',
			args: args,
			callback: function(r) {
				if(r.message) {
