...
def check_if_advance_entry_modified(args):
	...
	if args.voucher_type == "Journal Entry":
		...
		party_account_field = ("paid_from"
			if verp.get_party_account_type(args.party_type) == 'Receivable' else "paid_to")
...
def get_coa(doctype, parent, is_root, chart=None):
	from verp.accounts.doctype.account.chart_of_accounts.chart_of_accounts import build_tree_from_json
...
def check_if_stock_and_account_balance_synced(posting_date, company, voucher_type=None, voucher_no=None):
	if not cint(verp.is_perpetual_inventory_enabled(company)):
		return
		...
		if abs(account_bal - stock_bal) > 0.1:
			...
			frappe.msgprint(
				...
				primary_action={
					...
					'client_action': 'verp.route_to_adjustment_jv',
