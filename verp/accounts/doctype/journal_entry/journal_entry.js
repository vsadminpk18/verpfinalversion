	...
	voucher_type: function(frm){
		...
		if((!(frm.doc.accounts || []).length) || ((frm.doc.accounts || []).length === 1 && !frm.doc.accounts[0].account)) {
			if(in_list(["Bank Entry", "Cash Entry"], frm.doc.voucher_type)) {
				...
				return frappe.call({
					type:"GET",
					method: "verp.accounts.doctype.journal_entry.journal_entry.get_opening_accounts",
					args: {
						"company": frm.doc.company
					},
					callback: function(r) {
	...
	setup_queries() {
		...
		me.frm.set_query("reference_name", "accounts", function(doc, cdt, cdn) {
			...
			if(jvd.reference_type==="Payroll Entry") {
				return {
					query: "verp.payroll.doctype.payroll_entry.payroll_entry.get_payroll_entries_for_jv",
				};
			}
...
frappe.ui.form.on("Journal Entry Account", {
	party: function(frm, cdt, cdn) {
		...
		if(!d.account && d.party_type && d.party) {
			...
			return frm.call({
				method: "verp.accounts.doctype.journal_entry.journal_entry.get_party_account_and_balance",
				child: d,
				args: {
					company: frm.doc.company,
	...
	cost_center: function(frm, dt, dn) {
		verp.journal_entry.set_account_balance(frm, dt, dn);
	},
...
$.extend(verp.journal_entry, {
	set_account_balance: function(frm, dt, dn) {
		...
		if(d.account) {
			...
			return frappe.call({
				method: "verp.accounts.doctype.journal_entry.journal_entry.get_account_balance_and_party_type",
				args: {
					account: d.account,
