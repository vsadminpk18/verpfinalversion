...
from verp.accounts.party import get_party_account, get_due_date
from verp.accounts.utils import get_account_currency, get_fiscal_year
from verp.stock.doctype.purchase_receipt.purchase_receipt import update_billed_amount_based_on_po
from verp.stock import get_warehouse_account_map
from verp.accounts.general_ledger import make_gl_entries, merge_similar_entries, make_reverse_gl_entries
...
from verp.accounts.doctype.sales_invoice.sales_invoice import validate_inter_company_party, update_linked_doc,\
...
from verp.stock.doctype.purchase_receipt.purchase_receipt import get_item_account_wise_additional_cost
...
class PurchaseInvoice(BuyingController):
	...
	def on_submit(self):
		...
		if self.update_stock == 1:
			...
			from verp.stock.doctype.serial_no.serial_no import update_serial_nos_after_submit
		...
		for item in self.get("items"):
			if item.is_fixed_asset:
				...
				if not self.update_stock:
					...
					if item.item_tax_amount and not cint(verp.is_perpetual_inventory_enabled(self.company)):
						asset_eiiav_currency = get_account_currency(eiiav_account)
						gl_entries.append(self.get_gl_dict({
							"account": eiiav_account,
							"against": self.supplier,
