...
frappe.ui.form.on("BOM Item", {
	item_code: (frm, cdt, cdn) => {
		verp.crop.update_item_rate_uom(frm, cdt, cdn);
	},
	qty: (frm, cdt, cdn) => {
		verp.crop.update_item_qty_amount(frm, cdt, cdn);
	},
	rate: (frm, cdt, cdn) => {
		verp.crop.update_item_qty_amount(frm, cdt, cdn);
	}
...
verp.crop.update_item_rate_uom = function(frm, cdt, cdn) {
	...
	material_list.forEach((material) => {
		frm.doc[material].forEach((item, index) => {
			if (item.name == cdn && item.item_code){
				frappe.call({
					method:'verp.agriculture.doctype.crop.crop.get_item_details',
					args: {
...
verp.crop.update_item_qty_amount = function(frm, cdt, cdn) {
	let material_list = ['materials_required', 'produce', 'byproducts'];
	material_list.forEach((material) => {
		frm.doc[material].forEach((item, index) => {
			if (item.name == cdn){
