...
from verp.non_profit.doctype.donation.test_donation import create_donor, create_mode_of_payment, create_donor_type
from verp.non_profit.doctype.donation.donation import create_donation
from verp.non_profit.doctype.membership.test_membership import setup_membership, make_membership
from verp.non_profit.doctype.member.member import create_member
...
class TestTaxExemption80GCertificate(unittest.TestCase):
	...
	def test_membership_80g_certificate(self):
		...
		member_doc = create_member(frappe._dict({
			...
			'email': "_test_member_verp@example.com",
			'plan_id': plan.name
		}))
