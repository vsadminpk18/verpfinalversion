...
class TestStudentGroup(unittest.TestCase):
	...
	def test_in_group(self):
		...
		self.assertRaises(verp.education.StudentNotInGroupError,
			verp.education.validate_student_belongs_to_group, last_student, doc.name)

		# safe, don't throw validation
		verp.education.validate_student_belongs_to_group(doc.students[0].student, doc.name)
