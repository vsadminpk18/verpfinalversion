	...
	onload: function(frm) {
		...
		if (!frm.__islocal) {
			frm.set_query('student', 'students', function() {
				return{
					query: 'verp.education.doctype.student_group.student_group.fetch_students',
					filters: {
						'academic_year': frm.doc.academic_year,
	...
	refresh: function(frm) {
		if (!frm.doc.__islocal) {
			...
			frm.add_custom_button(__('Add Guardians to Email Group'), function() {
				frappe.call({
					method: 'verp.education.api.update_email_group',
					args: {
	...
	get_students: function(frm) {
		if (frm.doc.group_based_on == 'Batch' || frm.doc.group_based_on == 'Course') {
			...
			if (frm.doc.academic_year) {
				frappe.call({
					method: 'verp.education.doctype.student_group.student_group.get_students',
					args: {
						'academic_year': frm.doc.academic_year,
						'academic_term': frm.doc.academic_term,
						'group_based_on': frm.doc.group_based_on,
