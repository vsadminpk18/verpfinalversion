...
import frappe
import copy
from verp.controllers.item_variant import (ItemVariantExistsError,
		copy_attributes_to_variant, get_variant, make_variant_item_code, validate_item_variant_attributes)
from verp.setup.doctype.item_group.item_group import (get_parent_item_groups, invalidate_cache_for)
...
class Item(WebsiteGenerator):
	...
	def set_metatags(self, context):
		...
		context.metatags['og:site_name'] = 'VERP'

	def set_shopping_cart_data(self, context):
		from verp.shopping_cart.product_info import get_product_info_for_website
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue("verp.stock.doctype.item.item.update_variants",
...
def invalidate_item_variants_cache_for_website(doc):
	from verp.portal.product_configurator.item_variants_cache import ItemVariantsCacheManager
