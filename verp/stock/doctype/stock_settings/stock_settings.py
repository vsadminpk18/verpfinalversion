...
class StockSettings(Document):
	def validate(self):
		...
		from verp.setup.doctype.naming_series.naming_series import set_by_naming_series
	...
	def validate_clean_description_html(self):
		if int(self.clean_description_html or 0) \
			...
			# changed to text
			frappe.enqueue('verp.stock.doctype.stock_settings.stock_settings.clean_all_descriptions', now=frappe.flags.in_test)
