...
{% include 'verp/stock/landed_taxes_and_charges_common.js' %};
...
frappe.ui.form.on('Stock Entry', {
	...
	set_serial_no: function(frm, cdt, cdn, callback) {
		...
		frappe.call({
			method: "verp.stock.get_item_details.get_serial_no",
			args: {"args": args},
			callback: function(r) {
				if (!r.exe && r.message){
...
frappe.ui.form.on('Stock Entry Detail', {
	...
	barcode: function(doc, cdt, cdn) {
		...
		if (d.barcode) {
			frappe.call({
				method: "verp.stock.get_item_details.get_item_code",
...
verp.stock.StockEntry = class StockEntry extends verp.stock.StockController {
	...
	scan_barcode() {
		let transaction_controller= new verp.TransactionController({frm:this.frm});
...
verp.stock.select_batch_and_serial_no = (frm, item) => {
	...
	frappe.require("assets/verp/js/utils/serial_no_batch_selector.js", function() {
		new verp.SerialNoBatchSelector({
			frm: frm,
			item: item,
...
function check_should_not_attach_bom_items(bom_no) {
  return (
    bom_no === undefined ||
    (verp.stock.bom && verp.stock.bom.name === bom_no)
  );
}

$.extend(cur_frm.cscript, new verp.stock.StockEntry({frm: cur_frm}));
