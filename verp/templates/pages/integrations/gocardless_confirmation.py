...
from verp.verp_integrations.doctype.gocardless_settings.gocardless_settings import gocardless_initialization, get_gateway_controller
...
def create_mandate(data):
	...
	if frappe.db.exists("GoCardless Mandate", mandate):
		...
		verp_customer = frappe.db.get_value(reference_doc.reference_doctype, reference_doc.reference_name, ["customer_name"], as_dict=1)

		try:
			frappe.get_doc({
			"doctype": "GoCardless Mandate",
			"mandate": mandate,
			"customer": verp_customer.customer_name,
