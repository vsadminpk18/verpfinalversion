...
import verp
import frappe.utils
from verp.demo.user import hr, sales, purchase, manufacturing, stock, accounts, projects, fixed_asset
from verp.demo.user import education as edu
from verp.demo.setup import education, manufacture, setup_data, healthcare, retail
"""
Make a demo

1. Start with a fresh account

bench --site demo.verp.dev reinstall

2. Install Demo

bench --site demo.verp.dev execute verp.demo.demo.make

3. If Demo breaks, to continue

bench --site demo.verp.dev execute verp.demo.demo.simulate
...
def simulate(domain='Manufacturing', days=100):
	...
	frappe.flags.company = verp.get_default_company()
	frappe.flags.mute_emails = True
