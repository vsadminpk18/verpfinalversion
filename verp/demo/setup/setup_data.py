...
from verp.demo.domains import data
...
def setup_holiday_list():
	...
	frappe.set_value("Company", verp.get_default_company(), "default_holiday_list", holiday_list.name)


def setup_user():
	frappe.db.sql('delete from tabUser where name not in ("Guest", "Administrator")')
	for u in json.loads(open(frappe.get_app_path('verp', 'demo', 'data', 'user.json')).read()):
...
def setup_user_roles(domain):
	user = frappe.get_doc('User', 'demo@verp.com')
...
def setup_mode_of_payment():
	company_abbr = frappe.get_cached_value('Company',  verp.get_default_company(),  "abbr")
...
def setup_account():
	...
	data = json.loads(open(frappe.get_app_path('verp', 'demo', 'data',
		'account.json')).read())
	for d in data:
