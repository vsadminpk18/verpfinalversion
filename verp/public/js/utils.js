	...
	hide_company: function() {
		if(cur_frm.fields_dict.company) {
			...
			if(companies.length === 1) {
				...
				if(!cur_frm.doc.company) cur_frm.set_value("company", verp.last_selected_company);
			}
		}
	},
...
$(document).on('app_ready', function() {
	frappe.call({
		method: 'verp.support.doctype.service_level_agreement.service_level_agreement.get_sla_doctypes',
		callback: function(r) {
			if (!r.message)
				return;
