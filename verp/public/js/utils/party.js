...
verp.utils.get_party_details = function(frm, method, args, callback) {
	...
	if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (!verp.utils.validate_mandatory(frm, "Posting / Transaction Date",
			args.posting_date, args.party_type=="Customer" ? "customer": "supplier")) return;
	}

	if (!verp.utils.validate_mandatory(frm, "Company", frm.doc.company, args.party_type=="Customer" ? "customer": "supplier")) {
		return;
	}
...
verp.utils.set_taxes_from_address = function(frm, triggered_from_field, billing_address_field, shipping_address_field) {
	if (frm.updating_party_details) return;

	if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (!verp.utils.validate_mandatory(frm, "Lead / Customer / Supplier",
