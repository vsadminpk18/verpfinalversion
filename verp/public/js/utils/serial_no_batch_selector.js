...
verp.SerialNoBatchSelector = class SerialNoBatchSelector {
	...
	make_dialog() {
		...
		let fields = [
			...
			},
			{
				...
				get_query: function() {
					return {
						query: "verp.controllers.queries.warehouse_query",
						filters: [
			...
			},
			{
				...
				click: () => {
					...
					let numbers = frappe.call({
						method: "verp.stock.doctype.serial_no.serial_no.auto_fetch_serial_number",
						args: {
							qty: qty,
	...
	get_serial_no_fields() {
		...
		if (me.frm.doc.doctype === 'POS Invoice' && !this.showing_reserved_serial_nos_error) {
			frappe.call({
				method: "verp.stock.doctype.serial_no.serial_no.get_pos_reserved_serial_nos",
				args: {
					filters: {
						item_code: me.item_code,
						warehouse: typeof me.warehouse_details.name == "string" ? me.warehouse_details.name : '',
...
function get_pending_qty_fields(me) {
	...
	const { qty_consumed_per_unit } = verp.stock.bom.items[item_code];
...
function check_can_calculate_pending_qty(me) {
	...
	const docChecks = doc.bom_no
		...
		&& verp.stock.bom
		&& verp.stock.bom.name === doc.bom_no;
