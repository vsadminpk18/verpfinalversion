...
frappe.provide("verp.taxes.flags");
...
frappe.ui.form.on(cur_frm.doctype, {
	setup: function(frm) {
		...
		$(frm.wrapper).on('grid-row-render', function(e, grid_row) {
			if(in_list(['Sales Taxes and Charges', 'Purchase Taxes and Charges'], grid_row.doc.doctype)) {
				verp.taxes.set_conditional_mandatory_rate_or_amount(grid_row);
			}
		});
	},
	onload: function(frm) {
	...
	taxes_on_form_rendered: function(frm) {
		verp.taxes.set_conditional_mandatory_rate_or_amount(frm.open_grid_row());
	},
...
if(!verp.taxes.flags[cur_frm.cscript.tax_table]) {
	verp.taxes.flags[cur_frm.cscript.tax_table] = true;
	...
	frappe.ui.form.on(cur_frm.cscript.tax_table, "charge_type", function(frm, cdt, cdn) {
		...
		if(open_form) {
			verp.taxes.set_conditional_mandatory_rate_or_amount(open_form);
		} else {
			// apply in current row
			verp.taxes.set_conditional_mandatory_rate_or_amount(frm.get_field('taxes').grid.get_row(cdn));
		}
	});
...
verp.taxes.set_conditional_mandatory_rate_or_amount = function(grid_row) {
	if(grid_row) {
