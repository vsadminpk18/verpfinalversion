...
frappe.utils.make_event_emitter(verp.hub);

verp.hub.Marketplace = class Marketplace {
	constructor({ parent }) {
		this.$parent = $(parent);
	...
	register_marketplace({company, company_description}) {
		frappe.call({
		    method: 'verp.hub_node.api.register_marketplace',
		    args: {
				company,
				company_description
			}
			...
			if (r.message && r.message.ok) {
				...
				this.update_hub_settings()
					.then(() => {
						...
						verp.hub.trigger('seller-registered');
					});
			}
		});
	}
			...
			.then(r => {
				...
				const d = new frappe.ui.Dialog({
					...
					primary_action({ users }) {
						...
						frappe.call('verp.hub_node.api.register_users', {
	...
	get_unregistered_users() {
		return frappe.call('verp.hub_node.api.get_unregistered_users')
	}
