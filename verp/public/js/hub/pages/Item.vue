...
export default {
	...
	mounted() {
		...
		// verp.hub.item_view_cache = verp.hub.item_view_cache || [];
		// if (verp.hub.item_view_cache.includes(this.hub_item_name)) {
		// 	return;
		// }
		...
		this.item_received.then(() => {
			setTimeout(() => {
				...
				// .then(() => {
				// 	verp.hub.item_view_cache.push(this.hub_item_name);
				// });
			}, 5000);
		});
	},
	methods: {
		...
		add_to_saved_items() {
			hub.call('add_item_to_user_saved_items', {
				...
				.then(() => {
					...
					verp.hub.trigger('action:item_save');
				})
				.catch(e => {
					console.error(e);
				});
		...
		add_to_featured_items() {
			hub.call('add_item_to_seller_featured_items', {
				...
				.then(() => {
					...
					verp.hub.trigger('action:item_feature');
				})
				.catch(e => {
					console.error(e);
				});
				...
				primary_action: ({ message }) => {
					...
					hub.call('send_message', {
						...
						.then(() => {
							...
							verp.hub.trigger('action:send_message');
						});
				}
			});
		},
		...
		update_details(values) {
			frappe.call('verp.hub_node.api.update_item', {
		...
		unpublish_item() {
			frappe.confirm(__('Unpublish {0}?', [this.item.item_name]), () => {
				frappe
					.call('verp.hub_node.api.unpublish_item', {
