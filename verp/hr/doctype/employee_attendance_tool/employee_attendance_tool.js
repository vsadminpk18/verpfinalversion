frappe.ui.form.on("Employee Attendance Tool", {
	...
	onload: function(frm) {
		...
		verp.employee_attendance_tool.load_employees(frm);
	},
...
verp.employee_attendance_tool = {
	load_employees: function(frm) {
		if(frm.doc.date) {
			frappe.call({
				method: "verp.hr.doctype.employee_attendance_tool.employee_attendance_tool.get_employees",
				args: {
					date: frm.doc.date,
				...
				callback: function(r) {
					if(r.message['unmarked'].length > 0) {
						...
						frm.EmployeeSelector = new verp.EmployeeSelector(frm, frm.employee_area, r.message['unmarked'])
					}
					else{
					...
					if(r.message['marked'].length > 0) {
						...
						frm.marked_employee = new verp.MarkedEmployee(frm, frm.marked_employee_area, r.message['marked'])
					}
					else{
...
verp.MarkedEmployee = class MarkedEmployee {
...
verp.EmployeeSelector = class EmployeeSelector {
	constructor(frm, wrapper, employee) {
		this.wrapper = wrapper;
		this.frm = frm;
		this.make(frm, employee);
	...
	make(frm, employee) {
		...
		mark_employee_toolbar.find(".btn-mark-present")
			...
			.on("click", function() {
				...
				frappe.call({
					method: "verp.hr.doctype.employee_attendance_tool.employee_attendance_tool.mark_employee_attendance",
					args:{
