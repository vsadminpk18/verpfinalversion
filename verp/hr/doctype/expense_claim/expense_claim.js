...
frappe.ui.form.on('Expense Claim Detail', {
	expense_type: function(frm, cdt, cdn) {
		...
		return frappe.call({
			method: "verp.hr.doctype.expense_claim.expense_claim.get_expense_claim_account_and_cost_center",
			args: {
...
frappe.ui.form.on("Expense Claim", {
	setup: function(frm) {
		...
		frm.set_query("expense_approver", function() {
			return {
				query: "verp.hr.doctype.department_approver.department_approver.get_approvers",
				filters: {
					employee: frm.doc.employee,
	...
	onload: function(frm) {
		if (frm.doc.docstatus == 0) {
			return frappe.call({
				method: "verp.hr.doctype.leave_application.leave_application.get_mandatory_approval",
				args: {
					doctype: frm.doc.doctype,
				},
				callback: function(r) {
	...
	make_payment_entry: function(frm) {
		...
		if(frm.doc.__onload && frm.doc.__onload.make_payment_via_journal_entry) {
			method = "verp.hr.doctype.expense_claim.expense_claim.make_bank_entry";
		}
		return frappe.call({
			method: method,
			args: {
	...
	employee_name: function(frm) {
		verp.expense_claim.set_title(frm);
	},
	...
	get_advances: function(frm) {
		...
		if (frm.doc.employee) {
			return frappe.call({
				method: "verp.hr.doctype.expense_claim.expense_claim.get_advances",
				args: {
					employee: frm.doc.employee
				},
