...
frappe.ui.form.on('Patient Appointment', {
	...
	refresh: function(frm) {
		...
		frm.set_query('service_unit', function() {
			return {
				query: 'verp.controllers.queries.get_healthcare_service_units',
				filters: {
		...
		if (frm.is_new()) {
			frm.page.set_primary_action(__('Check Availability'), function() {
				if (!frm.doc.patient) {
					...
					frappe.call({
						method: 'verp.healthcare.doctype.patient_appointment.patient_appointment.check_payment_fields_reqd',
			...
			if (frm.doc.procedure_template) {
				frm.add_custom_button(__('Clinical Procedure'), function(){
					frappe.model.open_mapped_doc({
						method: 'verp.healthcare.doctype.clinical_procedure.clinical_procedure.make_procedure',
						frm: frm,
					});
				}, __('Create'));
				...
				frm.add_custom_button(__('Therapy Session'),function(){
					frappe.model.open_mapped_doc({
						method: 'verp.healthcare.doctype.therapy_session.therapy_session.create_therapy_session',
						frm: frm,
					})
				}, 'Create');
			} else {
				frm.add_custom_button(__('Patient Encounter'), function() {
					frappe.model.open_mapped_doc({
						method: 'verp.healthcare.doctype.patient_appointment.patient_appointment.make_encounter',
						frm: frm,
					});
				}, __('Create'));
			}
	...
	set_payment_details: function(frm) {
		frappe.db.get_single_value('Healthcare Settings', 'automate_appointment_invoicing').then(val => {
			if (val) {
				frappe.call({
					method: 'verp.healthcare.utils.get_service_item_and_practitioner_charge',
					args: {
						doc: frm.doc
					},
					callback: function(data) {
	...
	get_prescribed_therapies: function(frm) {
		if (frm.doc.patient) {
			frappe.call({
				method: "verp.healthcare.doctype.patient_appointment.patient_appointment.get_prescribed_therapies",
				args: {patient: frm.doc.patient},
				callback: function(r) {
					if (r.message) {
...
let check_and_set_availability = function(frm) {
	...
	function show_slots(d, fd) {
		if (d.get_value('appointment_date') && d.get_value('practitioner')) {
			...
			frappe.call({
				method: 'verp.healthcare.doctype.patient_appointment.patient_appointment.get_availability_data',
				args: {
...
let get_prescribed_procedure = function(frm) {
	if (frm.doc.patient) {
		frappe.call({
			method: 'verp.healthcare.doctype.patient_appointment.patient_appointment.get_procedure_prescribed',
			args: {patient: frm.doc.patient},
			callback: function(r) {
...
let update_status = function(frm, status){
	...
	frappe.confirm(__('Are you sure you want to cancel this appointment?'),
		function() {
			frappe.call({
				method: 'verp.healthcare.doctype.patient_appointment.patient_appointment.update_status',
