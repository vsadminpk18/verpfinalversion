	...
	this.page.main.on('click', '.btn-more', function() {
		...
		if (me.page.main.find('.'+docname).parent().find('.document-html').attr('data-fetched') == '1') {
			...
			if (doctype && docname) {
				...
				frappe.call({
					method: 'verp.healthcare.utils.render_doc_as_html',
					args:{
						doctype: doctype,
						docname: docname,
...
let setup_filters = function(patient, me) {
	...
	frappe.xcall(
		'verp.healthcare.page.patient_history.patient_history.get_patient_history_doctypes'
	).then(document_types => {
...
let get_documents = function(patient, me, document_types="", selected_date_range="") {
	...
	frappe.call({
		'method': 'verp.healthcare.page.patient_history.patient_history.get_feed',
		args: filters,
		callback: function(r) {
			let data = r.message;
			if (data.length) {
...
let show_patient_vital_charts = function(patient, me, btn_show_id, pts, title) {
	frappe.call({
		method: 'verp.healthcare.utils.get_patient_vitals',
		args:{
			patient: patient
		},
		callback: function(r) {
