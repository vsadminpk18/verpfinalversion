...
from verp.verp_integrations.utils import create_mode_of_payment
from verp.verp_integrations.doctype.mpesa_settings.mpesa_connector import MpesaConnector
from verp.verp_integrations.doctype.mpesa_settings.mpesa_custom_fields import create_custom_pos_fields
...
class MpesaSettings(Document):
	...
	def request_for_payment(self, **kwargs):
		...
		for i, amount in enumerate(request_amounts):
			...
			if frappe.flags.in_test:
				from verp.verp_integrations.doctype.mpesa_settings.test_mpesa_settings import get_payment_request_response_payload
	...
	def get_account_balance_info(self):
		...
		if frappe.flags.in_test:
			from verp.verp_integrations.doctype.mpesa_settings.test_mpesa_settings import get_test_account_balance_response
...
def generate_stk_push(**kwargs):
	...
	args = frappe._dict(kwargs)
	try:
		callback_url = get_request_site_address(True) + "/api/method/verp.verp_integrations.doctype.mpesa_settings.mpesa_settings.verify_transaction"
...
def get_account_balance(request_payload):
	...
	try:
		...
		callback_url = get_request_site_address(True) + "/api/method/verp.verp_integrations.doctype.mpesa_settings.mpesa_settings.process_balance_info"
