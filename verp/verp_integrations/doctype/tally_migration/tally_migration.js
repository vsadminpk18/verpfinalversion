...
frappe.ui.form.on("Tally Migration", {
	...
	verp_company: function (frm) {
		frappe.db.exists("Company", frm.doc.verp_company).then(exists => {
			if (exists) {
				frappe.msgprint(
					__("Company {0} already exists. Continuing will overwrite the Company and Chart of Accounts", [frm.doc.verp_company]),
				);
			}
		});
	},
...
verp.tally_migration.cleanDoc = (obj) => {
	...
	$.each(temp, function(key, value){
		if (value === "" || value === null){
			...
			$.each(value, function (k,v) { verp.tally_migration.cleanDoc(v); });
		}
	});
	return temp;
}
...
verp.tally_migration.get_html_rows = (logs, field) => {
	...
	let rows = logs
		.map(({ doc, exc }) => {
			...
			let create_button = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='verp.tally_migration.create_new_doc(${JSON.stringify(doc)})'>
					${__("Create Document")}
				</button>`

			let mark_as_unresolved = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='verp.tally_migration.unresolve(${JSON.stringify(doc)})'>
