...
verp.integrations.plaidLink = class plaidLink {
	...
	plaid_success(token, response) {
		...
		frappe.prompt({
			...
			frappe.xcall('verp.verp_integrations.doctype.plaid_settings.plaid_settings.add_institution', {
				token: token,
				response: response
			}).then((result) => {
				frappe.xcall('verp.verp_integrations.doctype.plaid_settings.plaid_settings.add_bank_accounts', {
					response: response,
					bank: result,
					company: me.company
				});
